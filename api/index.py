...
# Default route handler for static content
@app.route('/', defaults={'path': ''})
@app.route('/<path:path>')
def catch_all(path):
    """Catch all routes and return 404 or redirect to home"""
    if path.startswith('api/'):
        return jsonify({'error': 'Not Found'}), 404
    return jsonify({'message': 'HR Management System API', 'status': 'ok'}), 200
